...
public class 회전하는_큐_1021 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            int target_idx = deq.indexOf(arr[i]);
            int half_idx = deq.size() % 2 == 0 ? deq.size() / 2 - 1 : deq.size() / 2;

            // 찾으려는 숫자의 인덱스가, 중앙보다 왼쪽이면, 왼쪽으로 회전
            if (target_idx <= half_idx) {
                for (int j = 0; j < target_idx; j++) {
                    deq.offerLast(deq.pollFirst());
                    cnt++;
                }
            // 찾으려는 숫자의 인덱스가, 중앙보다 오른쪽이면, 오른쪽으로 회전
            } else {
